...
namespace FTP_Client
{
    public class FtpClient : IDisposable
    {
        ...
        public async Task DisconnectAsync()
        {
            ...
            {
                Debug.WriteLine("IOException thrown closing control connectin: " + e);
            }
            finally
            {
                m_Stream.Dispose();
        ...
        public async Task CreateDirectoryAsync(string path, bool force = true)
        {
            ...
            {
                Debug.WriteLine("CreateDirectory(\"{0}\", true): Create non-existent parent: {1}", path, path.GetFtpDirectoryName());
                await CreateDirectoryAsync(path.GetFtpDirectoryName());
            }
            else if (await DirectoryExistsAsync(path))
                return;

            Debug.WriteLine("CreateDirectory(\"{0}\", {1})", path.GetFtpPath(), force);
